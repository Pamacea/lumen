...
fn write_trend(md: &mut String, trend: &lumenx_score::TrendAnalysis) {
    writeln!(md, "## Trend Analysis").unwrap();
    writeln!(md).unwrap();

    // Direction indicator
    let (icon, description) = match trend.direction {
        lumenx_score::TrendDirection::Improving => ("📈", "Score is improving"),
        lumenx_score::TrendDirection::Stable => ("➡️", "Score is stable"),
        lumenx_score::TrendDirection::Declining => ("📉", "Score is declining"),
    };
...
fn write_recommendations(md: &mut String, dimensions: &DimensionScores) {
    ...
    let quick_wins: Vec<_> = all_improvements
        .iter()
        .filter(|(_, imp)| {
            matches!(imp.effort, lumenx_score::Effort::Trivial | lumenx_score::Effort::Low)
                && matches!(
                    imp.impact,
                    lumenx_score::Impact::Medium | lumenx_score::Impact::High
                )
        })
        .take(5)
        .collect();
...
fn format_metric(value: &lumenx_score::MetricValue) -> String {
    match value {
        ...
        lumenx_score::MetricValue::Boolean(v) => format!("{}", if *v { "Yes" } else { "No" }),
