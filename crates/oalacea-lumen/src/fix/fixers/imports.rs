...
impl ImportFixer {
        ...
        for pattern in &patterns {
            ...
            if pattern.contains("import * as") {
                ...
                let re = Regex::new(&format!(r#"import \* as {} from ['"].*?['"];"#, import_name))
                    .unwrap_or_else(|_| Regex::new(r"import\s+\*\s+as\s+\w+\s+from").unwrap());
                result = re.replace_all(&result, "").to_string();
            } else if pattern.contains('{') {
                // Handle named imports - fallback to a simple pattern if complex fails
                let re = Regex::new(&format!(
                    r#"import\s*\{{[^}}]*?{}[^}}]*?\}}\s*from\s*['"].*?['"];"#,
                    regex::escape(import_name)
                ))
                .unwrap_or_else(|_| Regex::new(r"import\s*\{[^}]*\}\s+from").unwrap());
    ...
    fn sort_go_imports(&self, content: &str) -> String {
        ...
        // Using lazy_static or once_cell would be better, but using unwrap_or_else for safety
