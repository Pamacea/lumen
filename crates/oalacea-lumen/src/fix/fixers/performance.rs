...
impl PerformanceFixer {
    ...
    fn add_js_async(&self, content: &str, issue: &ScoreIssue) -> String {
        ...
        let func_re = Regex::new(&format!(
            ...
            regex::escape(&func_name)
        )).unwrap_or_else(|_| Regex::new(r"function\s+\w+|const\s+\w+\s*=\s*\([^)]*\)\s*=>").unwrap());
        ...
        if func_re.is_match(&result) {
            ...
            if result.contains("fetch(") {
                let fetch_re = Regex::new(r#"fetch\(([^)]+)\)"#)
                    .unwrap_or_else(|_| Regex::new(r"fetch\(").unwrap());
    ...
    fn add_python_async(&self, content: &str, issue: &ScoreIssue) -> String {
        ...
        let func_re = Regex::new(&format!(
            r"def\s+{}\s*\(",
            regex::escape(&func_name)
        )).unwrap_or_else(|_| Regex::new(r"def\s+\w+\s*\(").unwrap());
    ...
    fn add_rust_async(&self, content: &str, issue: &ScoreIssue) -> String {
        ...
        let func_re = Regex::new(&format!(
            r"(pub\s+)?fn\s+{}\s*\(",
            regex::escape(&func_name)
        )).unwrap_or_else(|_| Regex::new(r"(pub\s+)?fn\s+\w+\s*\(").unwrap());
...
"#, func_name, func_name);
    ...
    fn add_python_caching(&self, content: &str, issue: &ScoreIssue) -> String {
        ...
        if func_re.is_match(&result) && !result.contains("@lru_cache") {
            ...
            // Note: This was previously !result.contains("import functools") && !result.contains("from functools")
            // but we need to handle the case where insert_str might fail
            if !result.contains("import functools") && !result.contains("from functools") {
                // Check if we can safely insert at position 0
                if result.len() < 100000 { // Prevent inserting into extremely large strings
    ...
    fn optimize_js_loop(&self, content: &str, issue: &ScoreIssue) -> String {
        ...
        let for_each_re = Regex::new(r#"(?P<array>\w+)\.forEach\((?P<callback>\([^)]*\)\s*=>\s*{)"#)
            .unwrap_or_else(|_| Regex::new(r#"\w+\.forEach\("#).unwrap());
