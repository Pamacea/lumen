...
impl QueryExecutor {
    ...
    pub fn exec_captures(&self, _query: &Query, _tree: &ParseTree, source: &str, _capture_name: &str) -> Vec<String> {
        ...
        let re_rust = regex::Regex::new(r"(?:pub\s+)?fn\s+([a-zA-Z_][a-zA-Z0-9_]*)")
            .unwrap_or_else(|_| regex::Regex::new(r"fn\s+\w+").unwrap());
        ...
        let re_js = regex::Regex::new(r"(?:export\s+)?[Ff]unction\s+([a-zA-Z_][a-zA-Z0-9_]*)")
            .unwrap_or_else(|_| regex::Regex::new(r"[Ff]unction\s+\w+").unwrap());
        ...
        let re_arrow = regex::Regex::new(r"(?:const|let)\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*\([^)]*\)\s*=>")
            .unwrap_or_else(|_| regex::Regex::new(r"(?:const|let)\s+\w+\s*=\s*\(").unwrap());
