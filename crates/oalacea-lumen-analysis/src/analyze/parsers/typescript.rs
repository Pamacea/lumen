...
use crate::analyze::parsers::{LanguageParser, ParsedCode, FunctionInfo, ClassInfo, ImportInfo, ExportInfo, VariableInfo, ParameterInfo};
...
impl TypeScriptParser {
    ...
    fn extract_functions(&mut self, tree: &ParseTree, source: &str) -> Vec<FunctionInfo> {
        let query = Query::function(self.language)
            .unwrap_or_else(|_| Query::new("function_definition", self.language).unwrap());
    ...
    /// Extract imports
    fn extract_imports(&mut self, tree: &ParseTree, source: &str) -> Vec<ImportInfo> {
...
impl Default for TypeScriptParser {
    fn default() -> Self {
        ...
        Self::new().unwrap_or_else(|_| TypeScriptParser {
            parser: Parser::new(AstLanguage::TypeScript)
                .unwrap_or_else(|_| Parser::new(AstLanguage::JavaScript).unwrap()),
            language: AstLanguage::TypeScript,
        })
    }
}
...
mod tests {
    ...
    fn test_parse_simple_function() {
        let mut parser = TypeScriptParser::new().expect("Failed to create TypeScript parser for test");
        let source = r#"
...
"#;
    ...
    fn test_analyze_any_usage() {
        let findings = TypeScriptAnalyzer::analyze_best_practices("let x: any = 5;")
            .expect("Failed to analyze best practices");
