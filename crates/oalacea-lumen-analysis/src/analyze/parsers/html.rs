...
impl HtmlParser {
    ...
    pub fn new() -> LumenResult<Self> {
        // HTML parsing is regex-based, no tree-sitter dependency
        Ok(Self)
    }
    ...
    pub fn extract_elements(&self, source: &str) -> Vec<HtmlElement> {
        ...
        for (i, line) in source.lines().enumerate() {
            ...
            while let Some(idx) = line[pos..].find('<') {
                let tag_start = pos + idx + 1;
                let rest_from_tag = &line[tag_start..];

                if let Some(relative_end) = rest_from_tag.find(|c| c == ' ' || c == '>' || c == '/') {
                    let tag = &rest_from_tag[..relative_end];
                    if !tag.is_empty() && !tag.starts_with('!') && !tag.starts_with('/') {
                        ...
                        elements.push(HtmlElement {
                            ...
                            line: i + 1,
                            is_self_closing: line.len() > tag_start + relative_end &&
                                            line[tag_start + relative_end..].starts_with("/>"),
                            attributes: Vec::new(),
                            children_count: 0,
                        });
                    }
                ...
                // Move past this tag
                if let Some(close_idx) = line[tag_start..].find('>') {
                    pos = tag_start + close_idx + 1;
                } else {
                    break;
                }
            }
