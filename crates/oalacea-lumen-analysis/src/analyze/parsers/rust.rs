...
use crate::analyze::ast::{AstLanguage, Parser, Query};
use crate::analyze::ast::parser::ParseTree;
use crate::analyze::ast::query::QueryExecutor;
use crate::analyze::parsers::{LanguageParser, ParsedCode, FunctionInfo, ImportInfo, ParameterInfo};
use oalacea_lumen_core::LumenResult as Result;
...
impl RustParser {
    ...
    fn extract_functions(&mut self, tree: &ParseTree, source: &str) -> Vec<FunctionInfo> {
        let query = Query::function(AstLanguage::Rust)
            .unwrap_or_else(|_| Query::new("function_definition", AstLanguage::Rust).unwrap());
    ...
    pub fn extract_structs(&mut self, _tree: &ParseTree, source: &str) -> Vec<StructInfo> {
    ...
    pub fn extract_uses(&mut self, tree: &ParseTree, source: &str) -> Vec<UseInfo> {
...
impl Default for RustParser {
    fn default() -> Self {
        // Try to create parser, fall back to one with default language
        Self::new().unwrap_or_else(|_| RustParser {
            parser: Parser::new(AstLanguage::TypeScript)
                .unwrap_or_else(|_| Parser::new(AstLanguage::Rust).unwrap())
        })
    }
}
...
mod tests {
    ...
    fn test_parse_rust_function() {
        let mut parser = RustParser::new().expect("Failed to create Rust parser for test");
        let source = r#"
pub fn hello() -> String {
    "Hello".to_string()
}
"#;
        ...
        assert!(result.is_ok());
        let parsed = result.expect("Failed to parse test code");
        assert_eq!(parsed.functions.len(), 1);
    }

    #[test]
    fn test_find_unsafe() {
        let parser = RustParser::new().expect("Failed to create Rust parser for test");
        let source = r#"
fn main() {
...
"#;
    ...
    fn test_extract_structs() {
        let mut parser = RustParser::new().expect("Failed to create Rust parser for test");
        let tree = parser.parser.parse("pub struct Test;")
