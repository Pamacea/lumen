...
use crate::analyze::ast::{AstLanguage, Parser, Query, Traversal, TraversalOrder};
use crate::analyze::ast::query::QueryExecutor;
use oalacea_lumen_core::{LumenError, LumenResult, ProjectInfo};
use oalacea_lumen_core::scoring::{MetricValue, ScoreIssue};
use regex::Regex;
...
impl StaticAnalyzer {
    ...
    pub fn analyze(
        ...
        source_files.par_iter().for_each(|file_path| {
            ...
            match self.analyze_file(file_path, language, info) {
                Ok(result) => {
                    // Use expect with descriptive message - in parallel context this is acceptable
                    // as the mutex should only fail if another thread panicked while holding the lock
                    let mut stats = language_stats.lock()
                        .expect("Failed to lock language_stats mutex in parallel analysis");
                    ...
                    let mut issues = all_issues.lock()
                        .expect("Failed to lock all_issues mutex in parallel analysis");
        ...
        // safe to use expect here as we're outside the parallel context
        let mut all_issues = Arc::try_unwrap(all_issues)
            .map_err(|_| oalacea_lumen_core::LumenError::InternalError(
                "Failed to unwrap Arc: other references still exist".to_string()
            ))?
            .into_inner()
            .map_err(|_| oalacea_lumen_core::LumenError::InternalError(
                "Failed to get Mutex inner: mutex was poisoned".to_string()
            ))?;
        let language_stats = Arc::try_unwrap(language_stats)
            .map_err(|_| oalacea_lumen_core::LumenError::InternalError(
                "Failed to unwrap Arc: other references still exist".to_string()
            ))?
            .into_inner()
    ...
    fn collect_source_files(&self, info: &ProjectInfo) -> LumenResult<Vec<PathBuf>> {
        ...
        let extensions = match info.language {
            oalacea_lumen_core::Language::TypeScript => &["ts", "tsx"][..],
            oalacea_lumen_core::Language::JavaScript => &["js", "jsx"][..],
            oalacea_lumen_core::Language::Rust => &["rs"][..],
            oalacea_lumen_core::Language::Python => &["py"][..],
            oalacea_lumen_core::Language::Go => &["go"][..],
    ...
    fn analyze_metrics_fast(&self, tree: &tree_sitter::Tree, source: &str, language: AstLanguage) -> (f64, usize, usize) {
        ...
        let traversal = Traversal::new(
            crate::analyze::ast::AstNode::new(root, language),
            TraversalOrder::Pre,
        );
    ...
    fn calculate_nesting_depth(
        &self,
        node: &crate::analyze::ast::AstNode,
        source: &str,
    ) -> usize {
        let mut max_depth = 0;
        let mut current_depth = 0;

        fn visit(node: &crate::analyze::ast::AstNode, source: &str, current_depth: &mut usize, max_depth: &mut usize) {
            let kind = node.kind();
        ...
        for (i, line) in source.lines().enumerate() {
                ...
                if let Ok(re) = Regex::new(pattern) {
                    if re.is_match(line) {
                        issues.push(ScoreIssue {
                            ...
                            severity: oalacea_lumen_core::scoring::IssueSeverity::High,
                ...
                if let Ok(re) = Regex::new(pattern) {
                    if re.is_match(line) {
                        issues.push(ScoreIssue {
                            ...
                            severity: oalacea_lumen_core::scoring::IssueSeverity::Critical,
                            category: "security".to_string(),
            ...
            if line.contains(".innerHTML") || line.contains("dangerouslySetInnerHTML") {
                issues.push(ScoreIssue {
                    ...
                    severity: oalacea_lumen_core::scoring::IssueSeverity::Medium,
                    category: "security".to_string(),
            ...
            for (i, line) in source.lines().enumerate() {
                    ...
                    || line.contains(&format!("def {}", func))
                {
                    issues.push(ScoreIssue {
                        ...
                        severity: oalacea_lumen_core::scoring::IssueSeverity::Low,
...
pub fn analyze(project: &oalacea_lumen_core::Project) -> Vec<oalacea_lumen_core::scoring::ScoreIssue> {
