...
use oalacea_lumen_core::{LumenResult, Project};
use oalacea_lumen_core::scoring::ScoreIssue;
use regex::Regex;
use std::collections::HashMap;
use walkdir::WalkDir;

/// Helper function to create regex with safe fallback
fn make_regex(pattern: &str) -> Regex {
    Regex::new(pattern)
        .unwrap_or_else(|_| {
            // Fallback to a simple pattern that won't match anything
...
fn score_issue_from_uiux(issue: UIUXIssue) -> ScoreIssue {
    ScoreIssue {
        ...
        severity: match issue.severity {
            IssueSeverity::Info => oalacea_lumen_core::scoring::IssueSeverity::Info,
            IssueSeverity::Low => oalacea_lumen_core::scoring::IssueSeverity::Low,
            IssueSeverity::Medium => oalacea_lumen_core::scoring::IssueSeverity::Medium,
            IssueSeverity::High => oalacea_lumen_core::scoring::IssueSeverity::High,
            IssueSeverity::Critical => oalacea_lumen_core::scoring::IssueSeverity::Critical,
        },
...
fn analyze_layout(
    ...
    let display_flex = make_regex(r"display\s*:\s*(flex|inline-flex)\b");
    let display_grid = Regex::new(r"display\s*:\s*(grid|inline-grid)\b");
    let position_absolute = Regex::new(r"position\s*:\s*absolute\b");
    let position_fixed = Regex::new(r"position\s*:\s*fixed\b");
    let float_pattern = Regex::new(r"float\s*:\s*(left|right)\b");
    let z_index_pattern = Regex::new(r"z-index\s*:\s*(-?\d+)");
...
fn count_orphaned_absolute(
    ...
    let mut orphaned = 0;
    let position_absolute = Regex::new(r"position\s*:\s*absolute\b");
    let position_relative = Regex::new(r"position\s*:\s*(relative|absolute|fixed|sticky)\b");
...
fn analyze_responsive(
    ...
    let min_width_640 = Regex::new(r"@media[^{]*min-width[^{]*640");
    let min_width_768 = Regex::new(r"@media[^{]*min-width[^{]*768");
    let min_width_1024 = Regex::new(r"@media[^{]*min-width[^{]*1024");
    let max_width_pattern = Regex::new(r"@media[^{]*max-width");
    let min_width_pattern = Regex::new(r"@media[^{]*min-width");
    let viewport_meta = Regex::new(r#"<meta[^>]*name\s*=\s*['"]viewport['"]"#);

    let responsive_unit_pattern = Regex::new(r"(rem|em|%|vw|vh)\b");
    let px_pattern = Regex::new(r"\d+px\b");
...
fn analyze_accessibility(
    ...
    // Regex patterns
    let semantic_tags = Regex::new(r"<(header|nav|main|article|section|aside|footer|figure|figcaption)\b");
    let all_tags = Regex::new(r"<([a-z][a-z0-9]*)\b");
    let main_element = Regex::new(r"<main\b");
    let nav_element = Regex::new(r"<nav\b");
    let heading_pattern = Regex::new(r"<h([1-6])\b");
    let img_with_alt = Regex::new(r#"<img[^>]*alt\s*=\s*["'][^"']*["']"#);
    // Note: Can't use negative lookaround, so we'll count img tags and subtract those with alt
    let img_all = Regex::new(r"<img\b");
    let focus_style = Regex::new(r"(:focus|\.focus|focus-visible)");
    let aria_label = Regex::new(r#"aria-label\s*=\s*["']"#);
    let positive_tabindex = Regex::new(r#"tabindex\s*=\s*[1-9]"#);
    let skip_link = Regex::new(r#"skip(-?link)?"#);
...
fn analyze_react_components(
    ...
    let component_def = Regex::new(r"(?:function|const)\s+([A-Z][a-zA-Z0-9]*)\s*[\(|=]");
    let component_export = Regex::new(r"export\s+(?:default\s+)?(?:function|const)\s+([A-Z][a-zA-Z0-9]*)");
...
fn analyze_visual(css_files: &[(std::path::PathBuf, String)]) -> VisualAnalysis {
    ...
    // Regex patterns
    let css_var = Regex::new(r"var\(--[a-z-]+\)");
    let semantic_color_names = Regex::new(r"--color-(success|warning|error|danger|info|primary|secondary)");
    let font_family = Regex::new(r"font-family\s*:\s*[^;]+");
    let font_size_rems = Regex::new(r"font-size\s*:\s*([\d.]+rem)");
    let border_radius = Regex::new(r"border-radius\s*:\s*([\d.]+)(rem|px|em|%)");
    let magic_number = Regex::new(r":\s*[1-9]\d*px\b");
...
fn detect_spacing_grid(css_files: &[(std::path::PathBuf, String)]) -> bool {
    let spacing_pattern = Regex::new(r"(margin|padding|gap)\s*:\s*([\d.]+)(px|rem)");
