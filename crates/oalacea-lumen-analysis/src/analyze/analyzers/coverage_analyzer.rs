...
use oalacea_lumen_core::{LumenResult, ProjectInfo};
use oalacea_lumen_core::scoring::{MetricValue, ScoreIssue, IssueSeverity};
use regex::Regex;
...
impl CoverageAnalyzer {
    ...
    fn parse_jest_coverage(&self) -> LumenResult<CoverageAnalysis> {
        ...
        if !coverage_path.exists() {
            return Err(oalacea_lumen_core::LumenError::FileNotFound("coverage-final.json".into()));
        }
    ...
    fn parse_lcov(&self) -> LumenResult<CoverageAnalysis> {
        ...
        if !lcov_path.exists() {
            return Err(oalacea_lumen_core::LumenError::FileNotFound("lcov.info".into()));
        }
    ...
    fn parse_cargo_coverage(&self) -> LumenResult<CoverageAnalysis> {
        ...
        if !cobertura_path.exists() {
            return Err(oalacea_lumen_core::LumenError::FileNotFound("cobertura.xml".into()));
        }
    ...
    fn parse_python_coverage(&self) -> LumenResult<CoverageAnalysis> {
        ...
        if !coverage_path.exists() {
            return Err(oalacea_lumen_core::LumenError::FileNotFound(".coverage".into()));
        }
