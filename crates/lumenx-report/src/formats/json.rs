...
fn framework_to_json(framework: lumenx_core::Framework) -> serde_json::Value {
    json!({
...
fn language_to_json(language: lumenx_core::Language) -> serde_json::Value {
    json!({
...
fn test_runner_to_json(test_runner: lumenx_core::TestRunner) -> serde_json::Value {
    json!({
...
fn dimension_to_json(dim: &lumenx_score::DimensionScore) -> serde_json::Value {
    json!({
        "name": dim.name,
        "score": dim.score,
        "weight": dim.weight,
...
fn improvement_to_json(imp: &lumenx_score::Improvement) -> serde_json::Value {
    json!({
        "id": imp.id,
        "title": imp.title,
...
fn metadata_to_json(metadata: &lumenx_score::ScoreMetadata) -> serde_json::Value {
    json!({
        ...
        let dim_metrics: serde_json::Map<String, serde_json::Value> = dim
            ...
            .map(|(k, v)| {
                let json_value = match v {
                    lumenx_score::MetricValue::Float(val) => json!({ "type": "float", "value": val }),
                    lumenx_score::MetricValue::Integer(val) => json!({ "type": "integer", "value": val }),
                    lumenx_score::MetricValue::Percentage(val) => json!({ "type": "percentage", "value": val }),
                    lumenx_score::MetricValue::Duration(val) => json!({ "type": "duration_ms", "value": val }),
                    lumenx_score::MetricValue::Count(val) => json!({ "type": "count", "value": val }),
                    lumenx_score::MetricValue::Boolean(val) => json!({ "type": "boolean", "value": val }),
                    lumenx_score::MetricValue::String(val) => json!({ "type": "string", "value": val }),
                };
                (k.clone(), json_value)
            })
            .collect();
...
fn calculate_roi(
    effort: lumenx_score::Effort,
    impact: lumenx_score::Impact,
) -> f64 {
...
mod tests {
    use super::*;
    use lumenx_score::{DimensionScore, DimensionScores, Grade};
    ...
    fn mock_project_score() -> ProjectScore {
        use lumenx_score::{DimensionScore, ScoreMetadata, ScoreIssue, IssueSeverity};
