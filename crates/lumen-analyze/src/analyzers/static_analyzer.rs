...
use lumen_core::{LumenError, LumenResult, ProjectInfo};
use lumen_score::{MetricValue, ScoreIssue};
use regex::Regex;
...
impl StaticAnalyzer {
    ...
    fn collect_source_files(&self, info: &ProjectInfo) -> LumenResult<Vec<PathBuf>> {
        ...
        let extensions = match info.language {
            lumen_core::Language::TypeScript => &["ts", "tsx"][..],
            lumen_core::Language::JavaScript => &["js", "jsx"][..],
            lumen_core::Language::Rust => &["rs"][..],
            lumen_core::Language::Python => &["py"][..],
            lumen_core::Language::Go => &["go"][..],
        ...
        for (i, line) in source.lines().enumerate() {
                ...
                if let Ok(re) = Regex::new(pattern) {
                    if re.is_match(line) {
                        issues.push(ScoreIssue {
                            ...
                            severity: lumen_score::IssueSeverity::High,
                ...
                if let Ok(re) = Regex::new(pattern) {
                    if re.is_match(line) {
                        issues.push(ScoreIssue {
                            ...
                            severity: lumen_score::IssueSeverity::Critical,
                            category: "security".to_string(),
            ...
            if line.contains(".innerHTML") || line.contains("dangerouslySetInnerHTML") {
                issues.push(ScoreIssue {
                    ...
                    severity: lumen_score::IssueSeverity::Medium,
                    category: "security".to_string(),
                ...
                for (i, line) in source.lines().enumerate() {
                        ...
                        || line.contains(&format!("def {}", func))
                    {
                        issues.push(ScoreIssue {
                            ...
                            severity: lumen_score::IssueSeverity::Low,
...
pub fn analyze(project: &lumen_core::Project) -> Vec<lumen_score::ScoreIssue> {
