...
impl PythonParser {
    ...
    pub fn new() -> LumenResult<Self> {
        ...
        parser.set_language(&lang)
            .map_err(|e| lumenx_core::LumenError::ParseError(format!("Language error: {}", e)))?;
        Ok(Self { parser })
    }
    ...
    pub fn parse(&mut self, source: &str) -> LumenResult<ParsedCode> {
        let tree = self.parser.parse(source, None)
            .ok_or_else(|| lumenx_core::LumenError::ParseError("Failed to parse Python code".to_string()))?;
...
impl Default for PythonAnalyzer {
    fn default() -> Self {
        Self::new().expect("Failed to create Python analyzer")
    }
}
