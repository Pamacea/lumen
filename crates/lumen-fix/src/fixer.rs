...
impl AutoFixer {
    ...
    fn fix_unused_import(&self, issue: &ScoreIssue, dry_run: bool) -> LumenResult<FixResult> {
        let file = issue.file.as_ref().ok_or_else(|| {
            lumen_core::LumenError::InvalidPath("No file specified".to_string())
        })?;

        let file_path = self.project_root.join(file);
        let content = std::fs::read_to_string(&file_path).map_err(|e| {
            lumen_core::LumenError::FileNotFound(file_path.clone())
        })?;
...
impl From<lumen_score::IssueSeverity> for FixSeverity {
    fn from(severity: lumen_score::IssueSeverity) -> Self {
        match severity {
            lumen_score::IssueSeverity::Critical => FixSeverity::Critical,
            lumen_score::IssueSeverity::High => FixSeverity::High,
            lumen_score::IssueSeverity::Medium => FixSeverity::Medium,
            lumen_score::IssueSeverity::Low => FixSeverity::Low,
            lumen_score::IssueSeverity::Info => FixSeverity::Info,
        }
    }
}
...
mod tests {
    ...
    fn test_severity_conversion() {
        let severity = lumen_score::IssueSeverity::High;
