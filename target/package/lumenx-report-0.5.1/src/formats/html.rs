...
use lumenx_score::{DimensionScores, Grade, IssueSeverity, ProjectScore};
...
fn render_issue(issue: &lumenx_score::ScoreIssue) -> String {
    format!(
        r#"
...
fn render_trend(score: &ProjectScore) -> String {
    if let Some(trend) = &score.trend {
        let (icon, class, text) = match trend.direction {
            lumenx_score::TrendDirection::Improving => ("📈", "improving", "Score is improving"),
            lumenx_score::TrendDirection::Stable => ("➡️", "stable", "Score is stable"),
            lumenx_score::TrendDirection::Declining => ("📉", "declining", "Score is declining"),
        };
...
fn render_recommendations(dimensions: &DimensionScores) -> String {
    ...
    all_improvements.sort_by(|a, b| {
        ...
        let impact_value = |imp: &lumenx_score::Impact| -> u8 {
            match imp {
                lumenx_score::Impact::VeryHigh => 5,
                lumenx_score::Impact::High => 4,
                lumenx_score::Impact::Medium => 3,
                lumenx_score::Impact::Low => 2,
                lumenx_score::Impact::Minimal => 1,
            }
        };
    ...
    let quick_wins: Vec<_> = all_improvements
        ...
        .filter(|(_, imp)| {
            matches!(
                imp.effort,
                lumenx_score::Effort::Trivial | lumenx_score::Effort::Low
            ) && matches!(imp.impact, lumenx_score::Impact::Medium | lumenx_score::Impact::High)
        })
        .take(5)
        .collect();
...
fn format_metric_value(value: &lumenx_score::MetricValue) -> String {
    match value {
        lumenx_score::MetricValue::Float(v) => format!("{:.2}", v),
        lumenx_score::MetricValue::Integer(v) => format!("{}", v),
        lumenx_score::MetricValue::Percentage(v) => format!("{:.1}%", v),
        lumenx_score::MetricValue::Duration(v) => format!("{}ms", v),
        lumenx_score::MetricValue::Count(v) => format!("{}", v),
        lumenx_score::MetricValue::Boolean(v) => (if *v { "Yes" } else { "No" }).to_string(),
        lumenx_score::MetricValue::String(v) => v.clone(),
    }
}
