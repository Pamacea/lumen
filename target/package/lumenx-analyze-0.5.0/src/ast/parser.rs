...
impl Parser {
    ...
    pub fn new(language: AstLanguage) -> Result<Self> {
        ...
        parser.set_language(&ts_lang)
            .map_err(|e| lumenx_core::LumenError::ConfigError(format!("Failed to set language: {}", e)))?;
    ...
    pub fn parse(&mut self, source: &str) -> Result<Tree> {
        self.parser
            .parse(source, None)
            .ok_or_else(|| lumenx_core::LumenError::ParseError("Failed to parse source code".to_string()))
    }

    /// Parse a file
    pub fn parse_file(&mut self, path: &Path) -> Result<(Tree, String)> {
        let source = std::fs::read_to_string(path)
            .map_err(|e| lumenx_core::LumenError::Io(e))?;
